<!--SCRIPT JWUERY-->
		// view pop up bình chọn sản phẩm
		$('#btnLuuDanhGiaSP').click(function(){
			$('#idFormDanhGiaSP').css({'display':'none'});	
		});
		$('#btnDanhGiaSP').click(function(){
			var state = $('#idFormDanhGiaSP').css('display');
			//alert(state);
			...
			if (state === 'none')
			{
				...
				pos_top = 	pos_top + 25;
				//alert(pos_top);						
				$('#idFormDanhGiaSP').css({'display':'block','left':pos_left,'top':pos_top,'position': 'absolute'});		
			}
			else
			{
				//alert(state);
